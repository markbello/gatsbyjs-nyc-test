...
8. Give walkthrough of file structure
...
9. Change the metadata inside `gatsby-config.js` *(shortcut ga-meta)*
	```
...
11. Change the content in `pages/index.js` *(shortcut ga-index)*
  ```js
  const IndexPage = () => (
  <Layout>
  ...
  ```
12. Introduce `gatsby-image` and `gatsby-plugin-sharp`: `gatsby-plugin-sharp` provides image optimization including lazy loading and dynamic image resizing, and is added through `gatsby-config.js`. `gatsby-image` provides a React `<Img />` component leveraging `gatsby-plugin-sharp`, which replaces the `<img />` tag
...
13. Walk through `components/image.js`
14. Add `gatsby-logo.png` to the repo
15. Create `components/logo.js` with *ga-logo* for the entire file
  ```js
  ...
  */
  ...
  export default Logo
  ```
16. Add logo to `components/header.js`. Replace entire file with *ga-header*
```js
...
- ...be creative
```
17. For Markdown transformation `npm install --save gatsby-transformer-remark`
18. Add `gatsby-transformer-remark` and new src path resolution to `gatsby-config.js` with *ga-config*
```js
module.exports = {
  siteMetadata: {
...
```
19. Add shortcut *ga-node* to `gatby-node.js` to create pages from `.md` files
```js
...
```
18. Create `src/templates/blog-post.js` with *ga-blog*
```js
import React from "react"
...
```
20. Add `<Link />` to `pages/index.js`. Use *ga-link*
```js
<Link to={`/feature-requests`}>Feature Requests</Link>
```
21. Add/commit/push to Github
22. PR into `master`
23. Netlify will run Gatsby Build
24. ...
25. PROFIT